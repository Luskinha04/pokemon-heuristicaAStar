...
def desenhar_mapa(
    ...
    for x in range(len(mapa)):
        for y in range(len(mapa[0])):
            ...
            if tipo == "V":
                imagem = sprites_lava[frame_lava % len(sprites_lava)]
            else:
                imagem = sprite_terrenos.get(tipo)  # Busca imagem referente ao tipo
...
def main():
    ...
    while rodando:
        ...
        # Atualiza o frame da lava para simular animação
